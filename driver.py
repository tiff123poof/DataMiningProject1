...
cement = DATA[:, 0]  # Cement column

# Function to categorize feature into Low, Medium, High
def categorize_feature(feature):
    bins = np.percentile(feature, [0, 33, 66, 100])
    return np.digitize(feature, bins[1:-1], right=True)  # 0=Low,1=Medium,2=High

cement_levels = categorize_feature(cement)

# Boxplot for Cement
plt.figure(figsize=(8,5))
plt.boxplot([TARGETS[cement_levels==0], TARGETS[cement_levels==1], TARGETS[cement_levels==2]],
            labels=['Low Cement', 'Medium Cement', 'High Cement'])
plt.ylabel('Compressive Strength (MPa)')
# plt.title('Compressive Strength by Cement Content')
plt.show()
